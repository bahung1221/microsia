...
class Transporter {
   ...
   */
  static makeOutgoingResponse(handler) {
    return new OutgoingResponse(handler, this)
  }
   ...
   * @return {Object}
   */
  static makeOutgoingRequest(subject, body, caller) {
  ...
  funcCompose(fnList) {
     ...
     * @api public
     */
    return function executable(req, res, done) {
      let index = -1
      function dispatch (i) {
        ...
        try {
          return fn
            ? Promise.resolve(fn(req, res, dispatch.bind(null, i + 1)))
            : Promise.resolve()
        } catch (err) {
  ...
  request(subject, body, caller) {
    ...
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        reject(new Error(`Request to ${caller.name} service was timed out`))
