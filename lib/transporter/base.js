...
class Transporter {
  constructor(opt = {}) {
    this.opt = Object.assign({}, OPTINAL_CONFIGS, opt)
    this.replyCounter = 0
  }
  ...
  subscribe(subject, func, caller) {
    eventBus.on(subject, payload => {
      ...
      const response = this.makeOutgoingResponse((data) => {
  ...
  request(subject, body, caller) {
    ...
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        reject(new Error(`Request to local ${caller.name} service was timed out`))
        eventBus.off(replyTo, handler)
      }, this.opt.timeout)
